...
def build_lstm(word_embeddings, len_voc, word_emb_dim, N, args, freeze=False):
	...
	for i in range(N):
		...
		#post_pred_ans_out[i] = T.sum(T.stack([post_out, pred_ans_out[i]], axis=2), axis=2)
	
	#errors = [None]*N
	#for i in range(N):
	#	errors[i] = T.sum(lasagne.objectives.squared_error(pred_ans_out[i], ans_out[i]), axis=1)

	#loss = T.sum(lasagne.objectives.squared_error(pred_ans_out[0], ans_out[0]))
	...
	l_pred_ans_out = [None]*N
	l_pred_ans_dense = [None]*N
	utility_pred_ans = [None]*N
	for i in range(N):
		l_pred_ans_out[i] = lasagne.layers.InputLayer(shape=(None, args.hidden_dim), input_var=pred_ans_out[i])
		#l_pred_ans_out[i] = lasagne.layers.InputLayer(shape=(None, args.hidden_dim), input_var=post_pred_ans_out[i])
		l_pred_ans_dense[i] = lasagne.layers.DenseLayer(l_pred_ans_out[i], num_units=1,\
                                                    	nonlinearity=lasagne.nonlinearities.sigmoid,\
														W=l_utility_post_dense.W,\
														b=l_utility_post_dense.b)
		utility_pred_ans[i] = lasagne.layers.get_output(l_pred_ans_dense[i])
...
#def get_test_pred(utilities, errors)
#	marginalized_utilities = [0.0]*len(utilities)
#	for i in range(utilities):
#		marginalized_utilities[i] = sum([])
...
def validate(val_fn, utility_val_fn, fold_name, epoch, fold, utility_fold, args):
	...
	lstring = '%s: epoch:%d, cost:%f, utility_cost:%f, acc:%f, utility_acc:%f time:%d' % \
				(fold_name, epoch, cost*1.0/num_batches, utility_cost*1.0/num_batches, \
					corr*1.0/total, utility_corr*1.0/utility_total, time.time()-start)
	print lstring
