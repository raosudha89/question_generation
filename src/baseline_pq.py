...
def generate_data(post_sents,  ques_list, args):
	...
	return data_post_sents, data_post_sent_masks, data_ques_list, data_ques_masks_list
...
def build_lstm(word_embeddings, len_voc, word_emb_dim, N, args, freeze=False):
	...
	pq_out = [None]*N
	for i in range(N):
		pq_out[i] = T.mean(T.stack([post_out, ques_out[i]], axis=2), axis=2)
	...
	loss = 0.0
	for i in range(N):
		l_pq_out[i] = lasagne.layers.InputLayer(shape=(None, args.hidden_dim), input_var=pq_out[i])
		l_pq_dense[i] = lasagne.layers.DenseLayer(l_pq_out[i], num_units=1, nonlinearity=lasagne.nonlinearities.sigmoid)
		pq_preds[i] = lasagne.layers.get_output(l_pq_dense[i])
	...
	all_params = post_params + post_emb_params + ques_params + ques_emb_params
	...
	train_fn = theano.function([post_sents, post_sent_masks, ques_list, ques_masks_list, labels], \
									[loss] + pq_preds, updates=updates)
	dev_fn = theano.function([post_sents, post_sent_masks, ques_list, ques_masks_list, labels], \
									[loss] + pq_preds,)
...
def shuffle(q, qm, l):
	for i in range(len(q)):
		all_data = zip(q[i], qm[i], l[i])
		random.shuffle(all_data)
		q[i], qm[i], l[i] = zip(*all_data)
	return q, qm, l

def iterate_minibatches(post_sents, post_sent_masks, ques_list, ques_masks_list, batch_size, shuffle=False):
	if shuffle:
	...
	for start_idx in range(0, post_sents.shape[0] - batch_size + 1, batch_size):
		...
		data.append([post_sents[excerpt], post_sent_masks[excerpt], ques_list[excerpt], ques_masks_list[excerpt]])
	return data
...
def validate(val_fn, fold_name, epoch, fold, args):
	...
	_lambda = 0.5
	N = args.no_of_candidates
	post_sents, post_sent_masks, ques_list, ques_masks_list = fold
	for p, pm, q, qm in iterate_minibatches(post_sents, post_sent_masks, ques_list, ques_masks_list, \
...
def main(args):
	...
	post_sents, post_sent_masks, ques_list, ques_masks_list = \
					generate_data(post_sent_vectors, ques_list_vectors, args)
	print 'done! Time taken: ', time.time() - start

	t_size = int(len(post_sents)*0.8)
	train = [post_sents[:t_size], post_sent_masks[:t_size], ques_list[:t_size], ques_masks_list[:t_size]]
	dev = [post_sents[t_size:], post_sent_masks[t_size:], ques_list[t_size:], ques_masks_list[t_size:]]
