...
def build_lstm(content_list, content_masks_list, N, max_len, word_embeddings, word_emb_dim, hidden_dim, len_voc, batch_size):
	...
	for i in range(1, N):
		...
		l_lstms[i] = lasagne.layers.LSTMLayer(l_emb_, hidden_dim, mask_input=l_mask_,\
...
def build_utility_lstm(utility_posts, utility_post_masks, posts, post_masks, ans_list, ans_masks_list, \
	...
	post_ans_preds, params = build_concat_lstm(posts, post_masks, ans_list, ans_masks_list, \
												N, post_max_len, ans_max_len, \
												word_embeddings, word_emb_dim, hidden_dim, len_voc)

	return utility_preds, post_ans_preds, params

def build_concat_lstm(posts, post_masks, ans_list, ans_masks_list, \
					   N, post_max_len, ans_max_len, word_embeddings, word_emb_dim, hidden_dim, len_voc):
	...
	ans_out = [None]*N
	l_ans_in = lasagne.layers.InputLayer(shape=(batch_size, ans_max_len), input_var=ans_list[0])
	l_ans_mask = lasagne.layers.InputLayer(shape=(batch_size, ans_max_len), input_var=ans_masks_list[0])
	#l_ans_emb = lasagne.layers.EmbeddingLayer(l_ans_in, len_voc, word_emb_dim, W=word_embeddings)
	l_ans_emb = lasagne.layers.EmbeddingLayer(l_ans_in, len_voc, hidden_dim, W=lasagne.init.GlorotNormal('relu'))
	l_ans_lstm = lasagne.layers.LSTMLayer(l_ans_emb, hidden_dim, mask_input=l_ans_mask, )
	l_post_ans_concat = lasagne.layers.ConcatLayer([l_lstm, l_ans_lstm], axis=1)
	l_post_ans_dense = lasagne.layers.DenseLayer(l_post_ans_concat, num_units=hidden_dim,\
													nonlinearity=lasagne.nonlinearities.rectify)
	l_post_ans_dense2 = lasagne.layers.DenseLayer(l_post_ans_dense, num_units=1,\
													nonlinearity=lasagne.nonlinearities.sigmoid)
	post_ans_preds[0] = lasagne.layers.get_output(l_post_ans_dense2)
	
	for i in range(1, N):
		l_ans_in_ = lasagne.layers.InputLayer(shape=(batch_size, ans_max_len), input_var=ans_list[i])
		l_ans_mask_ = lasagne.layers.InputLayer(shape=(batch_size, ans_max_len), input_var=ans_masks_list[i])
		#l_ans_emb_ = lasagne.layers.EmbeddingLayer(l_in_, len_voc, word_emb_dim, W=word_embeddings)
		l_ans_emb_ = lasagne.layers.EmbeddingLayer(l_ans_in_, len_voc, hidden_dim, W=lasagne.init.GlorotNormal('relu'))
		l_ans_lstm_ = lasagne.layers.LSTMLayer(l_ans_emb_, hidden_dim, mask_input=l_ans_mask_,\
									ingate=lasagne.layers.Gate(W_in=l_ans_lstm.W_in_to_ingate,\
																W_hid=l_ans_lstm.W_hid_to_ingate,\
																b=l_ans_lstm.b_ingate,\
																nonlinearity=l_ans_lstm.nonlinearity_ingate),\
									outgate=lasagne.layers.Gate(W_in=l_ans_lstm.W_in_to_outgate,\
																W_hid=l_ans_lstm.W_hid_to_outgate,\
																b=l_ans_lstm.b_outgate,\
																nonlinearity=l_ans_lstm.nonlinearity_outgate),\
									forgetgate=lasagne.layers.Gate(W_in=l_ans_lstm.W_in_to_forgetgate,\
																W_hid=l_ans_lstm.W_hid_to_forgetgate,\
																b=l_ans_lstm.b_forgetgate,\
																nonlinearity=l_ans_lstm.nonlinearity_forgetgate),\
									cell=lasagne.layers.Gate(W_in=l_ans_lstm.W_in_to_cell,\
																W_hid=l_ans_lstm.W_hid_to_cell,\
																b=l_ans_lstm.b_cell,\
																nonlinearity=l_ans_lstm.nonlinearity_cell),\
									peepholes=False,\
									)
		l_post_ans_concat_ = lasagne.layers.ConcatLayer([l_lstm_, l_ans_lstm_])
		l_post_ans_dense_ = lasagne.layers.DenseLayer(l_post_ans_concat_, num_units=hidden_dim,\
													nonlinearity=lasagne.nonlinearities.rectify)
		l_post_ans_dense2_ = lasagne.layers.DenseLayer(l_post_ans_dense_, num_units=1,\
													nonlinearity=lasagne.nonlinearities.sigmoid)
		post_ans_preds[i] = lasagne.layers.get_output(l_post_ans_dense2_)
		
	params += lasagne.layers.get_all_params(l_post_ans_dense2, trainable=True)
	
	return post_ans_preds, params
...
def build_evpi_model(word_embeddings, len_voc, word_emb_dim, N, args, freeze=False):
	...
	post_lstm = build_lstm_posts(posts, post_masks, args.post_max_len, \
								word_embeddings, word_emb_dim, args.hidden_dim, len_voc, args.batch_size)	
	ques_lstms = build_lstm(ques_list, ques_masks_list, N, args.ques_max_len, \
							word_embeddings, word_emb_dim, args.hidden_dim, len_voc, args.batch_size)
	ans_lstms = build_lstm(ans_list, ans_masks_list, N, args.ans_max_len, \
							word_embeddings, word_emb_dim, args.hidden_dim, len_voc, args.batch_size)
	
	pqa_preds = [None]*N
	l_post_ques_ans_concat = lasagne.layers.ConcatLayer([post_lstm, ques_lstms[0], ans_lstms[0]], axis=1)
	l_post_ques_ans_dense = lasagne.layers.DenseLayer(l_post_ques_ans_concat, num_units=args.hidden_dim,\
														nonlinearity=lasagne.nonlinearities.rectify)
	...
	for i in range(1, N):
		l_post_ques_ans_concat_ = lasagne.layers.ConcatLayer([post_lstm, ques_lstms[i], ans_lstms[i]], axis=1)
		l_post_ques_ans_dense_ = lasagne.layers.DenseLayer(l_post_ques_ans_concat_, num_units=args.hidden_dim,\
															nonlinearity=lasagne.nonlinearities.rectify,\
	...
	# for i in range(N):
	# 	for j in range(N):
	# 		all_sq_errors[i*N+j] = T.sum(lasagne.objectives.squared_error(post_ques_preds[i], ans_out[j]), axis=1)
	# 		loss += T.sum(lasagne.objectives.squared_error(post_ques_preds[i], ans_out[j])*labels[:,i,None])
	...
	all_params = post_lstm_params + ques_lstm_params + ans_lstm_params + post_ques_dense_params2 + utility_params
