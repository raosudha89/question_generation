...
def build_utility_lstm(utility_post_list, utility_post_masks_list, post_max_sents, max_len, word_embeddings, word_emb_dim, hidden_dim, len_voc, batch_size):
	...
	l_lstm = lasagne.layers.LSTMLayer(l_drop_in, hidden_dim, mask_input=l_mask, )
	...
	for i in range(1, post_max_sents):
		...
		l_drop_in_ = lasagne.layers.DropoutLayer(l_emb_, p=0.2)
		l_lstm_ = lasagne.layers.LSTMLayer(l_drop_in_, hidden_dim, mask_input=l_mask_,\
...
def build_evpi_model(word_embeddings, len_voc, word_emb_dim, args, freeze=False):
	...
	#l_utility_post_drop = lasagne.layers.DropoutLayer(l_utility_post_dense, p=0.5)
	for i in range(5):
...
def main(args):
	...
	print 'Size of utility training data: ', utility_train_size + utility_train_triples_size
	print 'Size of utility dev data: ', utility_dev_size + utility_dev_triples_size
