...
def generate_data(posts, post_max_len, questions_list, question_max_len):
	...
	no_of_questions = len(questions_list[0])	
	data_questions_list = np.zeros((len(questions_list), no_of_questions, question_max_len), dtype=np.int32)
	data_question_masks_list = np.zeros((len(questions_list), no_of_questions, question_max_len), dtype=np.int32)
...
def validate(val_fn, fold_name, epoch, fold, batch_size):
	...
	for p, pm, q, qm in iterate_minibatches(posts, post_masks, questions_list, question_masks_list, \
		...
		q_list = np.empty([no_of_questions, q.shape[0], q.shape[1]], dtype=np.int32)
		qm_list = np.empty([no_of_questions, qm.shape[0], qm.shape[1]], dtype=np.float32)
		for i in range(no_of_questions):
			q_list[i] = q[:,i].astype('int32')
			qm_list[i] = qm[:,i].astype('float32')
		l = np.zeros([batch_size, no_of_questions], dtype=np.int32)
		l[:,0] = 1
...
def build_lstm(word_embeddings, len_voc, word_emb_dim, hidden_dim, no_of_questions, post_max_len, question_max_len, batch_size, learning_rate, freeze=False):
	...
	l_question_lstm = [None]*no_of_questions
	...
	question_out = [None]*no_of_questions
	for i in range(no_of_questions):
	...
	updates = lasagne.updates.adam(loss, emb_post_params + emb_question_params + all_post_params + all_question_params + [M], learning_rate=learning_rate)
...
def main(args):
	posts = p.load(open(args.posts, 'rb'))
	questions_list = p.load(open(args.questions_list, 'rb'))
	...
	posts, post_masks, questions_list, question_masks_list = generate_data(posts, args.post_max_len, questions_list, args.question_max_len)
	...
	no_of_questions = len(questions_list[0])
	print 'Candidate question list size: ', no_of_questions
	start = time.time()
	print 'compiling graph...'
	train_fn, val_fn = build_lstm(word_embeddings, vocab_size, word_emb_dim, args.hidden_dim, no_of_questions, args.post_max_len, args.question_max_len, args.batch_size, args.learning_rate, freeze=freeze)
	print 'done! Time taken: ', time.time()-start
...
if __name__ == '__main__':
	...
	argparser.add_argument("--posts", type = str)
	argparser.add_argument("--questions_list", type = str)
